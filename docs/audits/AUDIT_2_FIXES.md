...
| 4.4 | A7 | Flush compteurs : garde `status IN ('running','claimed')` + clamp `LEAST(...)` + anti-poison (bisect/per-row + drop loggé) + drain du canal avant flush final (C6 workers) | `src/workers/batch_updater.rs` | En cours | |
