...
| 4.2 | A2 | Ligne outbox `start` complétée dans la même tx que `mark_task_running` + gate start-before-end borné par fraîcheur | `src/workers/start_loop.rs`, `src/db/webhook_execution.rs`, `src/db/task_crud.rs` | En cours | |
