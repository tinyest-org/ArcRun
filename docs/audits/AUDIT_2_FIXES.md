...
| 4.5 | A3 | Paused : pause depuis Pending+Waiting (propagation inclut Paused), Running → 400, endpoint resume contextuel (`wait_* > 0 ? Waiting : Pending`), corriger la doc OpenAPI | `src/db/task_lifecycle.rs`, `src/workers/propagation.rs`, `src/handlers/task.rs`, `src/validation/task.rs` | En cours | **Décision utilisateur (2026-07-09)** : pause = Pending + Waiting (décréments et cascade-fail s'appliquent aux Paused ; une Paused ne passe jamais Pending d'elle-même) ; pause d'une Running interdite (400, « cancel it instead »). |
